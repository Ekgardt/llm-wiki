...
Finding K-C6 of the 2026-09-17 audit: the 300 s limit existed with no production
caller, so up to four servers lived until capacity eviction or process exit. The
manager keeps one clock for "idle": the session's own last-used instant, which
capacity eviction already orders by.
Research: docs/research/2026-09-17-sess-a-session-that-failed-to-close-or-start-is-tried-again.md
and docs/research/2026-09-18-lsp-one-clock-for-one-idea-of-idle.md
"""
...
def _age_session(session, seconds: float) -> None:
    """Make the session that owns a server look unused for that long."""
    assert session._process is not None
    with session._lock:
        session._last_used_monotonic = time.monotonic() - seconds
...
def test_the_next_request_closes_a_server_idle_past_the_limit(
    ...
    try:
        ...
        _age_session(idle, pyright_session._IDLE_SECONDS + 1.0)
...
def test_a_server_inside_the_limit_is_left_alone(
    ...
    try:
        ...
        _age_session(recent, pyright_session._IDLE_SECONDS - 1.0)
        manager.get(scopes[1], deadline=time.monotonic() + SHORT_TIMEOUT)
...
def test_the_session_being_asked_for_is_never_reaped(
    ...
    try:
        ...
        _age_session(session, pyright_session._IDLE_SECONDS + 1.0)
