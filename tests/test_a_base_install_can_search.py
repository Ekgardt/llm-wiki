...
20 modules failed to import and `recall` failed. PyYAML is now a base dependency;
numpy stays in the hybrid profile and is imported where vectors are computed. Research:
...
def test_the_answer_path_modules_load_without_the_hybrid_profile():
    code = (
        "import sys; sys.modules['numpy'] = None; "
        f"sys.path.insert(0, {str(SCRIPTS)!r}); import fact_keys, evidence_pruning, query_memory"
    )

    result = subprocess.run([sys.executable, "-c", code], capture_output=True, text=True, cwd=ROOT, check=False)
