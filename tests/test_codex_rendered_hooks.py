...
def test_original_template_remains_supported(tmp_path):
    assert _verdict(tmp_path, COMMAND) == (True, "runtime_hooks_active")


def test_rendered_hook_still_requires_enabled_and_unique_match(tmp_path):
    wanted = dict(eventName="Stop", matcher=None, command=COMMAND)
    hook = dict(wanted, command=_installed_command(tmp_path), enabled=False, trustStatus="trusted")
