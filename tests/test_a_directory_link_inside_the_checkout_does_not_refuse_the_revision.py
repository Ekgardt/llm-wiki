...
ENVIRONMENT_MODULE = ".venv/lib/python3/site-packages/dep.py"
...
def test_a_checkout_with_a_virtual_environment_link_has_a_revision_that_verifies(tmp_path):
    ...
    _directory_link_or_skip(root / ".venv" / "lib64", "lib")
    ...
    assert not [path for path in paths if path.startswith(".venv/lib64")]
...
def test_a_directory_link_that_leaves_the_checkout_is_still_refused(tmp_path):
    ...
    outside.mkdir()
    _directory_link_or_skip(root / ".venv" / "lib64", outside)
