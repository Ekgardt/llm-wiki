...
- **Детерминированный admission** — до журналирования или мутации Python требует полные нормализованные устойчивые bullets через единый ограниченный точный индекс и соблюдение lifecycle/word-границ, затем отклоняет дубликаты в live-corpus и текущем плане; счётчики провайдера не обходят admission, неоднозначные пары только попадают в отчёт
...
- **Темпоральные запросы** — строгие `--as-of YYYY-MM-DD` / `--since YYYY-MM-DD`; `as_of` проверяет timestamp/`valid_to` и применяет typed-provenance веса
...
- **Аудируемый repair установленного vault** — audit schema v4 создаёт запечатанный manifest с `approved: false` и приватный временный staging; apply принимает только одобренный оператором manifest, физически удаляет лишь строго проверенный мусор и после commit удаляет исходные staged-байты, поэтому у завершённой очистки нет backup или rollback
...
- **Сбор 2793 тестов стабилен между платформами; локальная проверка Windows: 2749 пройдено, 44 пропущено**. Число пропусков зависит от доступности опциональных Bash, PowerShell и symlink; CI нацелен на Ubuntu + Windows + macOS с Python 3.10 + 3.13.
...
4. Запускает тестовый набор (2793 собирается на всех платформах; локально на Windows: 2749 пройдено, 44 пропущено; число пропусков зависит от shell/symlink)
...
cd llm-wiki
uv sync
uv run pytest -q          # 2793 собрано; локально Windows: 2749 пройдено, 44 пропущено; skips зависят от среды
```
...
> **Методология**: 66 known-item запросов (точное совпадение заголовка + ключевые слова из summary, не LLM-парафраз) по 33 каноническим активным победителям selector; shadow- и неактивные страницы исключены. BM25-only режим (FTS5). Измеряет «может ли система найти страницу X по её заголовку или ключевым словам из summary?» — наиболее релевантная метрика для персонального извлечения знаний. Это **не** LoCoMo или LongMemEval (multi-session conversation recall). Числа конкурентов — из других датасетов и не сопоставимы напрямую. Результаты ниже получены в одном локальном запуске Windows / Python 3.14 от 2026-08-03; задержка зависит от машины. Воспроизведите: `benchmark/run_benchmark.py`.

| Метрика | LLM Wiki v3.4 + Unreleased | agentmemory | Zep | Mem0 |
|---------|---------------|-------------|-----|------|
...
| Латентность p50 | **36.4мс** | 14мс | 155мс | 880мс |
