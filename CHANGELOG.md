...
- Installed-memory repair now exposes separate read-only audit, validated backup-only, manifest-bound apply, and read-only verify stages. Only backup-only creates a manifest; mutating apply requires an explicit existing manifest and fails before runtime mutation when it is absent. It mutates only verified empty daily records and generated false feedback; meaningful daily content, duplicate notes, and orphan service sessions remain preserved or report-only. No production cleanup or live restart is claimed by this entry.
...
- Compile admission is deterministic before journaling or mutation: durable-section citations, lifecycle and word bounds, and live-corpus/same-plan duplicate checks are mandatory. Provider counters cannot bypass admission, and ambiguous pairs remain report-only.
...
- Windows Task Scheduler launches maintenance with `pythonw`, and maintenance child processes are created without console windows.

### Testing
- **1916 tests collected (platform-stable); local Windows verification: 1881 passed, 35 skipped.** Skip count varies with optional Bash, PowerShell, and symlink availability. Coverage includes OpenCode lifecycle injection and project attribution, mutation-only breadcrumbs, dual-format project-safe daily context, repeatable queue maintenance and provenance, deterministic compile admission, effect-receipt replay, bounded retired-store reactivation, windowless Windows maintenance, bounded and Unicode-scalar-safe stdin, durable compile journals and generation manifests, OS-backed compile and queue locks, complete project-claim inventory and bootstrap provenance, bounded hook/advisory/bootstrap/settings inputs, safe Claude/Codex hook migration, XDG-aware plugin installation, transactional installed-memory repair, and durable weekly failed-queue reporting. CI has not yet verified this Unreleased worktree.
