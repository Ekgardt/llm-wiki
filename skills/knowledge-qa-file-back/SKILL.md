...
If a question straddles both, default to `knowledge/notes/qa/` and link out to the relevant wiki concept(s) — it is easier to promote later via `bridge-promote-insight` than to demote.
...
5. **Cross-link**:
   ...
   - If the Q&A sits in notes but references a related concept, link to the wiki concept in `## Related`.
