...
description: Extract a reusable workflow from a recent successful task and save it as a draft playbook under memory/knowledge/workflows/. The crystallization is conservative — it only extracts steps that would apply to future tasks of the same shape, never one-off project specifics.
...
1. **Identify the source material**. If `$ARGUMENTS` is a file path, read it. Otherwise, inspect the most recent daily log under `memory/daily/` for today's date and find the most recent meaningful session block (one with `Tier: major` or substantial content).
...
3. **Check for duplicates**. Read `memory/knowledge/workflows/` (and `memory/knowledge/patterns/`) — if a similar workflow already exists, the result should be an UPDATE to that page, not a new sibling. The DEDUP-BEFORE-CREATE rule from compile_memory.py applies here.

4. **Draft the playbook** under `memory/knowledge/workflows/<descriptive-slug>.md` using this format:
   ...
   ## Evidence
   - Crystallized from `memory/daily/<date>.md` block at `[HH:MM:SS]` (verified to support the steps above).

   ## Related
   - [[memory/knowledge/patterns/<related-pattern>]]
   - [[memory/knowledge/decisions/<related-decision>]]
   ```
...
6. **Update the index**. After creating the playbook, add a line to `memory/index.md` under a new or existing `## Workflows` section, with the page stem and the one-sentence summary.

7. **Reciprocal backlinks**. Edit each `## Related` target to add a backlink to the new playbook (unless it already links back).

8. **Do NOT auto-promote to a skill**. `.claude/skills/` is for human-curated, stable workflows. Playbooks under `memory/knowledge/workflows/` are agent-authored drafts; they earn promotion to a SKILL.md only after explicit user review.
