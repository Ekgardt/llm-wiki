...
---
Review `$ARGUMENTS` if provided, otherwise review the whole `memory/` subsystem.

Procedure:
1. **Structural pass (scripted):**
   Run `uv run python scripts/lint_memory.py --structural-only`.
   The script writes a report to `$LLM_WIKI_STATE_ROOT/memory-reports/lint-YYYY-MM-DD.md` (default `$LLM_WIKI_STATE_ROOT/memory-reports\`, outside the vault) covering:
   - broken wikilinks across `memory/` and `wiki/`
...
2. **LLM review pass (over the scripted findings):**
   ...
   - Are there weak links between `memory/knowledge/` and `wiki/` that would benefit from `bridge-promote-insight`?
