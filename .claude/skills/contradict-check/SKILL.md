...
## When to use
- Before committing a batch of edits to `memory/knowledge/` or `wiki/concepts|syntheses|comparisons|connections|qa/`.
...
1. Run: `python scripts/lint_memory.py --contradictions $ARGUMENTS`.
   - `--scope all` (default) — both `memory/` and `wiki/`.
...
2. Open the generated report at `$LLM_WIKI_STATE_ROOT/memory-reports/lint-YYYY-MM-DD.md` (default `$LLM_WIKI_STATE_ROOT/memory-reports\`) and read the `## Contradictions` section.

3. For each finding:
   - If real: pick one page as canonical, update the other to defer to it, and add a `wiki/log.md` or `memory/log.md` entry noting the resolution.
...
if git diff --cached --name-only | grep -qE '^(memory/knowledge|wiki/(concepts|syntheses|comparisons|connections|qa))/'; then
