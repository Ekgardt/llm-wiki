...
Procedure:
1. Identify the relevant source file(s) in `inbox/` and/or `raw/`.
2. Inspect `wiki/index.md` and any likely existing pages first.
3. Decide whether to:
   - create new pages under `wiki/entities/`, `wiki/concepts/`, `wiki/syntheses/`, or `wiki/comparisons/`
...
5. Update `wiki/index.md` so the page is discoverable.
6. Append one concise line to `wiki/log.md` with date + what changed.
