...
**Description.** A human curates `raw/` inputs; an LLM incrementally compiles a markdown wiki (`knowledge/notes/`) with concept pages, entities, syntheses, and backlinks. Obsidian is the frontend. At ~100 articles the LLM answers by reading the wiki directly — no dedicated RAG. See [[Karpathy LLM Wiki Workflow]].
...
- Should memory entries be allowed to cite `raw/` directly, or only via `knowledge/notes/` pages?
