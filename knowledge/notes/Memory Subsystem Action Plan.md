...
1. **Extend `operating-model.md` with a Compile Procedure section.** **Done 2026-04-13.** Expanded 2026-04-14 with the explicit two-question boundary table and worked examples. Current state covers trigger, inclusion filter, five knowledge categories with definitions, `do not lift` exclusions, index-update procedure, and the `memory/` ↔ `knowledge/notes/` boundary check.
