...
The `.obsidian/` directory in the project root configures Obsidian to treat the whole repo as one vault — so `raw/`, `inbox/`, `wiki/`, `memory/`, and `outputs/` are all browsable from the same sidebar. Obsidian Web Clipper is the recommended path for capturing an article into `inbox/articles/` before it is compiled into `wiki/` and eventually moved to `raw/`.
