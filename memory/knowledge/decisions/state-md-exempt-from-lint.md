...
description: "`state.md` files under `wiki/projects/<slug>/` are added to `EDITORIAL_NAMES` in `lint_memory.py` and exempted from backlink-obligation and sparse-floor checks, for the same reason that `index.md` and"
timestamp: 2026-07-03T05:41:37
---
# state.md Exempt from Lint

One-sentence summary: `state.md` files under `wiki/projects/<slug>/` are added to `EDITORIAL_NAMES` in `lint_memory.py` and exempted from backlink-obligation and sparse-floor checks, for the same reason that `index.md` and `log.md` are exempt.

## Decision

**Date:** 2026-04-19

**Context:** Phase 1 of the Global Multi-Project Migration Plan introduced `wiki/projects/<slug>/state.md` as a "where we left off" page auto-updated by the SessionStart/SessionEnd hook system. Lint immediately flagged these pages for missing backlinks (they reference many pages but receive few) and risked flagging them as sparse (a fresh state.md from the template is under 200 words).
...
## Evidence
- `memory/daily/2026-04-19.md` [03:00:46] — decision recorded during Phase 1 execution

## Related
- [[memory/knowledge/concepts/editorial-notes-pattern]] — the broader category this decision extends
- [[memory/knowledge/decisions/flag-inferred-content-as-preliminary]] — complementary decision about content classification
