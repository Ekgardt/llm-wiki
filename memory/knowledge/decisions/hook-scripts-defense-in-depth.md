...
2. **Degenerate slug values** — `Path.resolve()` normally prevents `.`, `..`, or empty strings reaching `_compute_slug()`, but a future refactor (e.g., skipping resolve for performance) could break this. A slug of `"."` would write to `wiki/projects/./state.md`, which is a path traversal into the projects directory root.
...
## Evidence
- `memory/daily/2026-04-19.md` [17:09:56] — `LLM_WIKI_STATE_ROOT` bug surfaced
- `memory/daily/2026-04-19.md` [17:14:57] — slug guard rationale articulated
- `memory/daily/2026-04-19.md` [17:24:33] — both fixes confirmed and recorded as committed

## Related
- [[memory/knowledge/debugging/hook-errors-silent-without-state-root]] — deeper debugging entry for the state-root symptom
- [[memory/knowledge/patterns/b-sim-hook-testing]] — the testing technique that uncovered these gaps
