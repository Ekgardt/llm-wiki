...
1. `memory/knowledge/concepts/editorial-notes-pattern.md` used future-ish language for `memory/index.md` and `memory/log.md` editorial notes — both had been added on 2026-04-18.
2. `memory/knowledge/patterns/mirror-existing-pipelines.md` described the memory subsystem as having "the same immutable-raw / staged / compiled three-layer shape" as the core pipeline. Memory only has two layers (`daily/` + `knowledge/`); no staging equivalent exists. The wiki counterpart (`wiki/concepts/Pipeline Mirroring.md`) was already correct.
3. `wiki/syntheses/Global Multi-Project Migration Plan.md` Phase 2 Step 3 listed five skills to copy to user level. Only one (`knowledge-lookup`) was actually installed; the other four were intentionally kept project-level after scope reduction mid-execution. The step text was never updated after the decision.
...
4. **For append-only changelogs**: add an editorial disclaimer paragraph (see [[memory/knowledge/patterns/editorial-disclaimer-over-history-rewrite]]).
...
## Evidence
- `memory/daily/2026-04-19.md` [22:41:34] — review session surfaced all three instances above; fixes applied in the same session.

## Related
- [[memory/knowledge/patterns/audit-current-vs-intended]] — the audit-page discipline that distinguishes present-state facts (dated) from future aspirations.
- [[memory/knowledge/patterns/editorial-disclaimer-over-history-rewrite]] — the fix pattern for changelog entries that describe superseded behavior.
- [[memory/knowledge/concepts/provenance-rule-6]] — marking uncertainty explicitly is the root constraint; stale prospective claims violate it by presenting old intent as current fact.
