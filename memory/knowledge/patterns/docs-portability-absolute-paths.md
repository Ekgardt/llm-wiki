...
- Setup and re-setup guides (e.g., `wiki/projects/llm-wiki/re-setup.md`).
...
## Evidence
- `memory/daily/2026-04-19.md` [23:13:01] — audit v7 session; pattern applied to `README.md` and synthesis pages that contained bare `$LLM_WIKI_STATE_ROOT/` references.

## Related
- [[memory/knowledge/decisions/hook-scripts-defense-in-depth]] — `_resolve_state_root()` fallback is the *runtime* version of this same portability concern: when the env var is missing, fall back rather than hardcode.
- [[memory/knowledge/debugging/hook-errors-silent-without-state-root]] — what happens when `LLM_WIKI_STATE_ROOT` is absent entirely, making even the fallback insufficient without the explicit env block.
